...
var utilities = {
	...
	classifyObjectsBy: function(arr, key) {
		var sorted = {};
	...
	createThree: function(holder, init, update) {
		...
		function render() {
			...
			camera.orbitDist = 200 * (3 + Math.sin(.0005 * time.current));
			camera.orbitAngle = time.current * .002;
			var phi = .3;
			camera.position.x = camera.orbitDist * Math.cos(camera.orbitAngle) * Math.sin(phi);
			camera.position.y = camera.orbitDist * Math.sin(camera.orbitAngle) * Math.sin(phi);
			camera.position.z = camera.orbitDist * Math.cos(phi);
...
// From http://www.javascriptkit.com/javatutors/javascriptpromises.shtml
function loadPromiseImages(names, dataStore, context, onFinish) {
	console.log("Load images: " + names);


	var imgPromises = names.map(s => getImage(s, dataStore, context));
	Promise.all(imgPromises).then(function(urls) {
		finish();
	}).catch(function(urls) {
		console.log(urls + " not found") // returns array of images that failed to load

	});
}

function getPromiseImage(name, dataStore, context) {

	return new Promise(function(resolve, reject) {
		var img = new Image();
		img.onload = function() {
			dataStore.images[name] = img;

			context.drawImage(img, 0, 0, img.width, img.height);
			var imgData = context.getImageData(0, 0, img.width, img.height);
			//console.log(imgData);
			dataStore.imageData[name] = imgData;
			resolve(name);
		}
		img.onerror = function() {
			reject(name);
		}
		img.src = "css/img/" + name + ".jpeg";
	})
}


function createCanvasImg(img, holder) {
	var canvas = $("<canvas/>").appendTo(holder).css({
